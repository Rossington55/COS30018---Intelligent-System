...
def distance_betweeen(node1, node2):
    ...
    a = math.sin(dLat/2) * math.sin(dLat/2) + math.cos(deg_to_rad(LAT_corrected)) * \
        math.cos(deg_to_rad(LAT_corrected2)) * \
...
def aStarSearch(start, end, time, routes):
    ...
    while len(openList) > 0:
        ...
        for childNode in childNodes:
            ...
            childNode._distToStart = curNode.get_distToStart(
            ) + getCostofScats(curNode, childNode, time)
